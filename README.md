...
The RACE Survey App is a consistent, reproducible, and reliable local resource website for our boats while boats are out to sea.

This repository houses:

- The functions used to create the RACE Survey App (`sub_tasks/`, `run.R`)
- The HTML files that make up the website (`docs/`)
- The many files that are embedded in those websites (`files/`)

The files in total are too large to be housed within GitHub and are instead stored locally in the `files/` folder.

The **G: drive** version of the RACE Survey App  
(`G:\RACE_Survey_App`, must be connected to the VPN and have access to RACE_GF network) contains the most up to date version of this `files/` folder.

When first cloning/forking this repository:

1. Copy the `files/` folder from the G drive version of the app to your copy of the repository.
2. If you are working on this app, you should periodically copy the `files/` folder from the G drive version of the app to your copy of the repository so that you are up to date with the `files/` content.
...
The `run.R` script is the main engine that produces the application. It sources a sequence of scripts that are stored in the `sub_tasks/` folder. These scripts are numbered in the order they are called.

### General Flow of `run.R`

- `sub_tasks/01_import_R_packages.R`  
  Imports/installs R packages

- `sub_tasks/02_functions.R`  
...
- `sub_tasks/04_render_main_page.R`  
  Renders our main page called `index.html`

- In between these steps, each page is rendered

- `sub_tasks/05_render_species_pages.R`  
  Renders the species ID pages
