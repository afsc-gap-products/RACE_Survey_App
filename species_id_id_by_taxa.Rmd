...
grps <- guides_data %>%
  ...
  unique() %>%
  mutate(
    group_page = paste0("./zz_", make_clean_names(group), ".html"),
...
for (k in 1:nrow(grps)) {
  ...
  if (length(subgrps) > 1) { # generates subgroup level pages
    if(fam == "Rockfishes"){ #rockfishes are a strange case with nested subgroups
      ...
      page <- paste0("zz_", group_title, "_", str_remove(cols[ii], "/"), "_subpage_", jj, ".html")
    ...
    page <- paste0("zz_", group_title, "_subpage", kk, ".html")
  ...
  for (ll in 1:length(spp)) {
    ...
    page <- paste0("zz_", group_title, "_", gsub(" ", "_", species_page$species[1]), ".html")
